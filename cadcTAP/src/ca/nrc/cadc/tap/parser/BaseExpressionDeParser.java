 ...
 * @author jburke
 */
public class BaseExpressionDeParser extends ExpressionDeParser
        implements OperatorVisitor
{
    private static Logger log = Logger.getLogger(BaseExpressionDeParser.class);

    public BaseExpressionDeParser(SelectVisitor selectVisitor,
    ...
    {
        log.debug("visit(" + operator.getClass()
                .getSimpleName() + "): " + operator);
        if (operator.isNot())
        {
            buffer("NOT ");
        }
    ...
    public void visit(Concatenate concatenate)
    {
        ...
        buffer(concatenate.getOperator());
        buffer("'");
        buffer(concatenate.getSeparator());
        buffer("'");
        buffer(concatenate.getOperator());
    ...
    public void visit(Parenthesis parenthesis)
    {
        ...
        {
            final StringBuilder currBuffer = getBuffer();

            if (currBuffer.charAt(currBuffer.length() - 1) != ' ')
            {
                buffer(" ");
            }
    ...
    {
        log.debug("visit(" + caseExpression.getClass()
                .getSimpleName() + ") " + caseExpression);
        buffer("CASE ");
